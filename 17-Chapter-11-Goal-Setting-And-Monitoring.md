...
<mark>要让 AI 智能体真正有效且有目的性，它们需要的不仅仅是处理信息或使用工具的能力，还需要明确的方向感，并且要能够判断自己是否真正取得成功。目标设定与监控模式正是为此而生。这个模式通过为智能体设定具体目标，并为其配备跟踪进度、判断目标达成情况的手段，来解决上述问题。</mark>
...
<mark>想想计划一次旅行。你不会凭空出现在目的地。你需要决定想去哪里（目标状态），弄清楚从哪里出发（初始状态），考虑可用的选项（交通、路线、预算），然后规划出一系列步骤：订票、打包行李、前往机场/车站、登上交通工具、到达、找到住宿地等。这种逐步推进、考虑依赖关系和约束条件的过程，正是我们在智能体系统中所说的规划。</mark>
...
<mark>对于 AI 智能体而言，规划通常是指智能体接受一个高层目标后，自主（或半自主）生成一系列中间步骤或子目标。这些步骤既可以顺序执行，也可以按更复杂的流程执行，期间可能会涉及工具使用、路由或多智能体协作等其他模式。规划机制可能采用复杂的搜索算法、逻辑推理，或者利用大语言模型 (LLMs) 的能力——后者正变得越来越普遍，可以基于训练数据和任务理解来生成合理有效的计划。</mark>
...
<mark>良好的规划能力让智能体可以处理多方面的复杂请求，而不仅限于简单的单步查询。它使智能体能够通过重新规划来适应变化，并编排复杂的工作流程。这是一个基础模式，支撑着许多高级智能体行为，能将简单的反应式系统转变为主动追求既定目标的系统。</mark>
...
<mark>对于构建能够在复杂现实场景中自主可靠运行的智能体来说，目标设定与监控模式至关重要。以下是一些实际应用：</mark>
...
* **Customer Support Automation:** An agent's goal might be to "resolve customer's billing inquiry." It monitors the conversation, checks database entries, and uses tools to adjust billing. Success is monitored by confirming the billing change and receiving positive customer feedback. If the issue isn't resolved, it escalates.
   ...
   <mark>* **自动化客户支持：** 智能体的目标可能是"解决客户的账单查询"。它会监控对话、检查数据库条目，并使用工具调整账单。成功与否的判断依据包括：账单变更是否已确认，以及是否收到客户的积极反馈。如果问题未解决，智能体会将其升级处理。</mark>
...
<mark>为了说明目标设定与监控模式，我们提供了一个使用 LangChain 和 OpenAI API 的示例。这个 Python 脚本展示了一个自主 AI 智能体，专门用于生成和优化 Python 代码。它的核心功能是为特定问题生成解决方案，并确保符合用户定义的质量基准。</mark>
...
<mark>它采用"目标设定和监控"模式，不只是生成一次代码，而是进入创建、自我评估和改进的迭代循环。智能体的成功通过其自身的 AI 驱动判断来衡量，即生成的代码是否满足初始目标。最终输出的是一个经过润色、注释完整、可随时使用的 Python 文件。</mark>
...
<mark>要理解这个脚本，可以把它想象成一个被分配到项目的自主 AI 程序员（见图 1）。当您向 AI 提供详细的项目简报（即它需要解决的特定编程问题）时，整个过程就开始了。</mark>
...
```
<mark>译者注：[Colab 代码](https://colab.research.google.com/drive/1553L0BIxqhPFS6RHiLnF_ELjEkWZpqsC?usp=drive_open) 已维护在[此处](/codes/17-Chapter-11-Goal-Setting-and-Monitoring-Example.py)。</mark>
...
<mark>接到这个任务后，AI 程序员开始工作并生成代码初稿。然而，它不会立即提交这个初始版本，而是暂停下来执行一个关键步骤：严格的自我审查。它会一丝不苟地扮演自己的质量保证检查员，将自己的创作与您提供的质量检查清单逐项比较。检查完成后，它会对自己的进展给出一个简单、公正的评判：符合所有标准为"True"，未达标准为"False"。</mark>
...
<mark>如果评判结果为"False"，AI 也不会放弃。它会进入深思熟虑的修订阶段，利用自我批判的见解找出弱点，并智能地重写代码。这种起草、自我审查和优化的循环会持续进行，每次迭代都更接近目标。

这个过程会一直重复，直到 AI 满足每一个要求并最终达到"True"状态，或者达到预设的尝试次数上限——就像一个面对截止日期的开发者。一旦代码通过最终检查，脚本就会打包经过润色的解决方案，添加有用的注释，并保存为一个新的 Python 文件供使用。</mark>
...
<mark>**警告和注意事项：** 需要注意的是，这只是一个示例性说明，并非生产就绪的代码。在实际应用中，必须考虑以下几个因素：LLM 可能无法完全理解目标的真实含义，从而错误地判断自己已经成功；即使目标理解正确，模型也可能产生幻觉；尤其是当同一个 LLM 既负责编写代码又负责评判质量时，它更难发现自己已经走偏。</mark>
...
<mark>归根结底，LLM 无法自动生成完美无缺的代码，您仍需运行和测试生成的代码。此外，这个简单示例中的"监控"机制较为基础，存在进程永远无法结束的潜在风险。</mark> 
...
<mark>**充当专业代码评审员**，深度致力于生成整洁、正确且简单的代码。您的核心使命是通过确保每个建议都基于实际情况和最佳实践来消除代码"幻觉"。当我向您提供代码片段时，我希望您能做到：

-- **识别和纠正错误：** 指出任何逻辑缺陷、错误或潜在的运行时错误。
-- **简化和重构：** 在不牺牲正确性的前提下，提出改善代码可读性、性能和可维护性的修改。
-- **提供清晰的解释：** 对于每个建议的变更，引用整洁代码、性能或安全的原则，解释为什么它能改进代码。
-- **提供更正后的代码：** 显示您建议变更的前后对比，使改进更清晰。

您的反馈应该直接、建设性，并始终以提高代码质量为目标。</mark>
...
<mark>更健壮的做法是给一组智能体分配特定角色，从而分离这些关注点。例如，我使用 Gemini 构建了一个个人 AI 智能体团队，每个智能体都有特定角色：</mark>
...
<mark>在这个多智能体系统中，代码评审员与程序员智能体分离，拥有类似于示例中评判者的提示词，从而使评估更加客观。这种结构自然带来更好的实践效果，因为测试编写员智能体可以为同伴程序员生成的代码编写单元测试。</mark>
...
<mark>**是什么：** AI 智能体通常缺乏明确的方向，使它们无法有目的地行动，只能执行简单的反应式任务。如果没有定义目标，它们就无法独立处理复杂的多步骤问题或编排复杂的工作流程。此外，它们缺乏内嵌机制来判断自己的行动是否能带来成果。这限制了它们的自主性，使其无法在动态的现实场景中真正发挥作用——在这些场景中，仅仅执行任务是远远不够的。</mark>
...
<mark>**为什么：** 目标设定与监控模式通过将目的感和自我评估嵌入智能体系统，提供了一种标准化解决方案。它明确定义智能体要实现的清晰、可测量的目标，同时建立监控机制，持续跟踪智能体的进度以及环境状态与目标的对比情况。这创建了一个关键的反馈循环，使智能体能够评估自身表现、纠正路线，并在偏离成功路径时调整计划。通过实施这种模式，开发人员可以将简单的反应式智能体转变为主动、目标导向且能够自主可靠运行的系统。</mark>
...
<mark>本章重点讨论了目标设定与监控这一关键范式，阐述了这一概念如何将 AI 智能体从纯粹的反应式系统转变为主动的、目标驱动的实体。明确定义可测量目标和建立严格的监控程序以跟踪进度至关重要。实际应用展示了这一范式如何在客户服务、机器人技术等多个领域支持可靠的自主操作。通过一个概念性编码示例，我们说明了如何在结构化框架内实现这些原则，使用智能体指令和状态管理来指导和评估智能体实现其指定目标的能力。归根结底，为智能体配备制定和监督目标的能力，是构建真正智能和负责任的 AI 系统的基石。</mark>
